(ns marketentry.marketentryllm
  ...
  the UGA public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-ug-entity?=" (:has-ug-entity? e)
                        " ursb-verified?=" (:ursb-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-ug-entity? e))
                              (:has-ug-entity? e))
                          (or (not (:requires-ursb? e))
                              (:ursb-verified? e)))
                   0.9 0.3)}))
