    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(URSB/統一資格/e-GP/PPDA登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- ug-entity-missing-violations
...
(defn- ursb-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-ursb? true`, INDEPENDENTLY check
  `:ursb-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-ursb? e))
                 (not (true? (:ursb-verified? e))))
        [{:rule :ursb-unverified
          :detail (str subject " はURSB確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (ug-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (ursb-unverified-violations request st)
